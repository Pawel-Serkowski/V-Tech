...
const networkHost = document.querySelector('[data-canvas-host="network"]');
const plannerHost = document.querySelector('[data-canvas-host="planner"]');
const tabs = [...document.querySelectorAll("[data-tab]")];
const views = [...document.querySelectorAll("[data-view]")];
const satelliteName = document.querySelector("[data-satellite-name]");
const satelliteMeta = document.querySelector("[data-satellite-meta]");
const satelliteDescription = document.querySelector("[data-satellite-description]");
const satelliteHint = document.querySelector("[data-satellite-hint]");
const mqttTopic = document.querySelector("[data-mqtt-topic]");
const mqttRoute = document.querySelector("[data-mqtt-route]");
const mqttPayload = document.querySelector("[data-mqtt-payload]");
const mqttStatus = document.querySelector("[data-mqtt-status]");
...
const mqttMessages = [
  "{packet:sync, qos:2, bridge:handover}",
  "{packet:telemetry, qos:1, mode:relay}",
  "{packet:routing, qos:1, status:rebalance}",
  "{packet:science, qos:0, stream:compressed}",
];
...
function updateSatellitePanel(data) {
  ...
  if (!data) {
    satelliteName.textContent = "Wybierz satelite";
    satelliteMeta.textContent = "Kliknij obiekt na orbicie Ziemi lub Ksiezyca";
...
function createDynamicRoute(elapsed) {
  const earthBase = nodeLookup.get("earth-main-base");
  const moonBase = nodeLookup.get("moon-main-base");
  const earthNodes = earthRelayIds.map((id) => nodeLookup.get(id));
  const moonNodes = moonRelayIds.map((id) => nodeLookup.get(id));
  const allNodes = [earthBase, ...earthNodes, ...moonNodes, moonBase];
  const adjacency = new Map(allNodes.map((node) => [node.id, []]));

  function addDirectedEdge(fromNode, toNode) {
    ...
    const cost = fromNode.getWorldPosition().distanceTo(toNode.getWorldPosition());
    if (!Number.isFinite(cost) || cost <= 0) {
      return;
    }
    adjacency.get(fromNode.id).push({ id: toNode.id, cost });
  }

  earthNodes.forEach((earthNode) => {
    addDirectedEdge(earthBase, earthNode);
    earthNodes.forEach((otherEarthNode) => addDirectedEdge(earthNode, otherEarthNode));
    moonNodes.forEach((moonNode) => addDirectedEdge(earthNode, moonNode));
  });

  moonNodes.forEach((moonNode) => {
    moonNodes.forEach((otherMoonNode) => addDirectedEdge(moonNode, otherMoonNode));
    addDirectedEdge(moonNode, moonBase);
  });

  const earthBasePosition = earthBase.getWorldPosition();
  const earthCenterPosition = earthGroup.getWorldPosition(new THREE.Vector3());
  const moonCenterPosition = moonGroup.getWorldPosition(new THREE.Vector3());
  const baseDirection = earthBasePosition.clone().sub(earthCenterPosition).normalize();
  const moonDirection = moonCenterPosition.clone().sub(earthCenterPosition).normalize();
  const visibilityAlignment = baseDirection.dot(moonDirection);
  const minEarthRelays = visibilityAlignment < -0.2 ? 3 : visibilityAlignment < 0.35 ? 2 : 1;

  const startState = `${earthBase.id}|0|0`;
  const targetState = `${moonBase.id}|${minEarthRelays}|1`;
  const bestDistance = new Map([[startState, 0]]);
  ...
  while (queue.length > 0) {
    ...
    if (current.cost > (bestDistance.get(current.state) ?? Infinity)) {
      continue;
    }

    for (const edge of adjacency.get(current.nodeId) ?? []) {
      const nextNode = nodeLookup.get(edge.id);
      const earthVisited = Math.min(
        minEarthRelays,
        current.earthVisited + (earthRelayIds.includes(nextNode.id) ? 1 : 0)
      );
      const moonVisited = Math.min(1, current.moonVisited + (moonRelayIds.includes(nextNode.id) ? 1 : 0));
      const nextState = `${nextNode.id}|${earthVisited}|${moonVisited}`;
      const nextCost = current.cost + edge.cost;

      if (nextCost < (bestDistance.get(nextState) ?? Infinity)) {
        bestDistance.set(nextState, nextCost);
  ...
  const visibleRoute = [];
  if (bestDistance.has(targetState)) {
    let state = targetState;
    const seenStates = new Set();
    while (state && !seenStates.has(state)) {
      ...
      visibleRoute.unshift(node);
      state = previous.get(state);
    }
  } else {
    visibleRoute.push(earthBase);
  }

  const packetIndex = Math.floor(elapsed / 2.8) % mqttMessages.length;
  const routeComplete =
    visibleRoute.length >= 4 &&
    visibleRoute[0].id === earthBase.id &&
    visibleRoute.at(-1).id === moonBase.id &&
    visibleRoute.filter((node) => earthRelayIds.includes(node.id)).length >= minEarthRelays &&
    visibleRoute.some((node) => moonRelayIds.includes(node.id));

  return {
    nodes: visibleRoute,
    names: visibleRoute.map((node) => node.name),
    payload: mqttMessages[packetIndex],
    topic: `mqtt/relay/earth-moon/${visibleRoute.map((node) => node.id).join("/")}`,
    qos: 2,
    status: routeComplete
      ? `aktywny relay handover ziemia -> ksiezyc przez ${minEarthRelays} sat. Ziemi`
      : "szukanie poprawnej trasy relay",
  };
}

createBase(earthSurfaceGroup, {
  id: "earth-main-base",
...
earthSatellites.forEach((config, index) => {
  ...
  createSatellite(earthGroup, config);
});
...
moonSatellites.forEach((config, index) => {
  ...
  createSatellite(moonGroup, config);
});
...
function animate() {
  ...
  moonPivot.rotation.y = elapsed * 0.08;
  ...
  route.nodes.slice(0, -1).forEach((node, index) => {
    ...
    const pulse = (elapsed * (0.7 + index * 0.18)) % 1;
  ...
  const routeKey = route.names.join("|");
  if (mqttStatus.dataset.routeKey !== routeKey || mqttPayload.textContent !== `Payload: ${route.payload}`) {
...
function setupPlannerView() {
  ...
  plannerRenderer.setSize(window.innerWidth, window.innerHeight);
  ...
  const plannerCamera = new THREE.PerspectiveCamera(46, window.innerWidth / window.innerHeight, 0.1, 4000);
  ...
  const plannerUi = {
    leoAltitude: document.querySelector('[data-planner-input="leo-altitude"]'),
    flightDays: document.querySelector('[data-planner-input="flight-days"]'),
    perilune: document.querySelector('[data-planner-input="perilune"]'),
    leoAltitudeValue: document.querySelector('[data-planner-value="leo-altitude"]'),
    flightDaysValue: document.querySelector('[data-planner-value="flight-days"]'),
    periluneValue: document.querySelector('[data-planner-value="perilune"]'),
    run: document.querySelector("[data-planner-run]"),
    deltaV: document.querySelector('[data-result="delta-v"]'),
    vinf: document.querySelector('[data-result="vinf"]'),
    closest: document.querySelector('[data-result="closest"]'),
    tof: document.querySelector('[data-result="tof"]'),
  ...
  [plannerUi.leoAltitude, plannerUi.flightDays, plannerUi.perilune].forEach((input) =>
    input.addEventListener("input", updatePlannerLabels)
  );
  updatePlannerLabels();

  function resize() {
    plannerCamera.aspect = window.innerWidth / window.innerHeight;
