...
function createLiveSatelliteConfig(node, index, activeLoad) {
  ...
  const tiltHash = hashNumber(key, 19);

  const radiusBase = network === "moon" ? 1.45 : 2.8;
  const radiusSpan = network === "moon" ? 1.8 : 3.7;
  const radius = radiusBase + ((hash % 1000) / 1000) * radiusSpan;

  const eccentricity = 0.76 + ((hashNumber(key, 7) % 170) / 1000);
  const angle = ((hashNumber(key, 11) % 1000) / 1000) * Math.PI * 2;
  const direction = hash % 2 === 0 ? 1 : -1;
  const tiltX = -0.9 + ((tiltHash % 1800) / 1000);
  const tiltZ = -0.9 + ((hashNumber(key, 23) % 1800) / 1000);
  ...
  return {
    ...
    bodyColor,
    panelColor,
    orbitColor: network === "moon" ? "#9eb7ff" : "#69d8ff",
    orbitOpacity: network === "moon" ? 0.14 : 0.18,
  };
}
...
function createLiveBaseConfig(node, activeLoad) {
  const key = node.node_id;
  const network = classifyNetwork(node);
  const hash = hashNumber(key, 41);
  const latHash = hashNumber(key, 43);

  const longitude = ((hash % 3600) / 10) * (Math.PI / 180);
  const latitude = (-55 + (latHash % 1100) / 10) * (Math.PI / 180);
  const normal = [
...
function syncLiveNodes() {
  ...
  const signature = orderedNodes
    .map((node) => [node.node_id, node.node_type, node.orbit, node.location_label].join("|"))
    .join(";");
